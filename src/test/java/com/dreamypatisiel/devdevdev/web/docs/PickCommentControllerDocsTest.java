...
public class PickCommentControllerDocsTest extends SupportControllerDocsTest {
    ...
    void registerPickReply() throws Exception {
        ...
        ResultActions actions = mockMvc.perform(
                        post("/devdevdev/api/v1/picks/{pickId}/comments/{pickCommentId}/replies",
                                pick.getId(), pickComment.getId())
                                .contentType(MediaType.APPLICATION_JSON)
                                .header(AUTHORIZATION_HEADER, SecurityConstant.BEARER_PREFIX + accessToken)
                                .characterEncoding(StandardCharsets.UTF_8)
        ...
        // docs
        actions.andDo(document("register-pick-reply",
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()),
                requestHeaders(
                        headerWithName(AUTHORIZATION_HEADER).description("Bearer 엑세스 토큰")
                ...
                requestFields(
                        fieldWithPath("contents").type(STRING).description("픽픽픽 답글 내용(최소 1자 이상 최대 1,000자 이하)")
                ),
                responseFields(
                        fieldWithPath("resultType").type(STRING).description("응답 결과"),
                        fieldWithPath("data").type(OBJECT).description("응답 데이터"),
                        fieldWithPath("data.pickReplyId").type(NUMBER).description("픽픽픽 답글 아이디")
                )
        ));
    }
    ...
    void registerPickReplyBindException(String contents) throws Exception {
        ...
        // docs
        actions.andDo(document("register-pick-reply-bind-exception",
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()),
                requestHeaders(
                        headerWithName(AUTHORIZATION_HEADER).description("Bearer 엑세스 토큰")
    ...
    void modifyPickReply() throws Exception {
        ...
        // when // then
        ResultActions actions = mockMvc.perform(
                        patch("/devdevdev/api/v1/picks/{pickId}/comments/{pickCommentId}/replies/{pickReplyId}",
        ...
        // docs
        actions.andDo(document("modify-pick-reply",
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()),
                requestHeaders(
                        headerWithName(AUTHORIZATION_HEADER).description("Bearer 엑세스 토큰")
                ...
                pathParameters(
                        parameterWithName("pickId").description("픽픽픽 아이디"),
                        parameterWithName("pickCommentId").description("픽픽픽 댓글 아이디"),
                        parameterWithName("pickReplyId").description("픽픽픽 답글 아이디")
                ),
                responseFields(
                        fieldWithPath("resultType").type(STRING).description("응답 결과"),
                        fieldWithPath("data").type(OBJECT).description("응답 데이터"),
    ...
    void modifyPickReplyBindException(String contents) throws Exception {
        ...
        // docs
        actions.andDo(document("modify-pick-reply-bind-exception",
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()),
                requestHeaders(
                        headerWithName(AUTHORIZATION_HEADER).description("Bearer 엑세스 토큰")
    ...
    void deletePickReply() throws Exception {
        ...
        ResultActions actions = mockMvc.perform(delete(
                        "/devdevdev/api/v1/picks/{pickId}/comments/{pickCommentId}/replies/{pickReplyId}",
                        pick.getId(), pickComment.getId(), pickReply.getId())
                        .contentType(MediaType.APPLICATION_JSON)
                        .header(AUTHORIZATION_HEADER, SecurityConstant.BEARER_PREFIX + accessToken)
                        .characterEncoding(StandardCharsets.UTF_8))
        ...
        // docs
        actions.andDo(document("delete-pick-reply",
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()),
                requestHeaders(
                        headerWithName(AUTHORIZATION_HEADER).description("Bearer 엑세스 토큰")
    ...
    void deletePickReplyNotFoundException() throws Exception {
        ...
        // docs
        actions.andDo(document("delete-pick-reply-not-found-exception",
                preprocessRequest(prettyPrint()),
                preprocessResponse(prettyPrint()),
                requestHeaders(
                        headerWithName(AUTHORIZATION_HEADER).description("Bearer 엑세스 토큰")
